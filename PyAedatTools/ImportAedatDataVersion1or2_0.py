...
def ImportAedatDataVersion1or2_0(aedat):
    """
    Later ;)
    """
    ...
    if info['source'] == 'Das1':
        ...
        cameraMask = int('F', 16) # hex2dec('0FFF0000');0xffffffff
        cameraShiftBits = 0;
    ...
    # Special events
        ...
        if ('dataTypes' not in importParams or 'polarity' in importParams['dataTypes']) \
            ...
            outputData['polarity']['cam'] = np.array(np.right_shift( \
                np.bitwise_and(polarityData, cameraMask), -cameraShiftBits), 'uint16')
